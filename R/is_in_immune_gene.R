...
#' 
#' This function filters integration site data for immune-associated genes (within specified distance/threshold)
...
#' @param Distance Numeric, maximum distance to immune gene (default: 100000 bp)
#' @param threashold Numeric, minimum clone contribution threshold (default: 0.001)
#' @return ggplot object (dot plot of clone contribution for immune-associated genes)
#' @export
is_in_immune_gene=function(IS_raw, Distance=100000,threashold=0.001){
  ...
  immune_gene=EPS$immune_gene
  
  immune_sheet=IS_raw[IS_raw$nearest_gene_name %in% immune_gene, ]
  ...
  immune_sheet=filter_gene_by_pos(immune_sheet)
  immune_sheet=immune_sheet[immune_sheet$Clone_contribution>=threashold,]
  
  immune_gene_sum=base::nrow(immune_sheet)
  if (immune_gene_sum==0){
    p=ggplot2::ggplot()+
      ggplot2::geom_abline(intercept = 0)+
      ggplot2::annotate("text", x=0, y=0, label= "No immune gene")+
  ...
  immune_sheet$Sample=base::factor(immune_sheet$Sample, 
  ...
  if (base::length(base::unique(immune_sheet$Sample))>20){
    p=  ggplot2::ggplot(immune_sheet, ggplot2::aes(nearest_gene_name, Clone_contribution*100))+
      ggplot2::geom_point(ggplot2::aes(nearest_gene_name, Clone_contribution*100), size=5, alpha=0.6, shape=18)+
      ggplot2::theme_bw()+
      ggplot2::ylim(0, base::max(immune_sheet$Clone_contribution*100*1.1, 5, na.rm = TRUE))+
      ggplot2::scale_color_manual(values = mycol)+
      ggplot2::xlab('immune associated genes')+ggplot2::ylab('Clone contribution [%]')+
      ggplot2::theme(legend.position = "right")+
      ggplot2::labs(color='')+
      ggplot2::coord_flip()
  }
  
  if (base::length(base::unique(immune_sheet$Sample))>3 & base::length(base::unique(immune_sheet$Sample))<=20){
    p= ggplot2::ggplot(immune_sheet, ggplot2::aes(nearest_gene_name, Clone_contribution*100))+
      ...
      ggplot2::labs(color='')+
      ggplot2::coord_flip()
  } else if(base::length(base::unique(immune_sheet$Sample))<=3){
    p= ggplot2::ggplot(immune_sheet, ggplot2::aes(nearest_gene_name, Clone_contribution*100))+
      ggplot2::geom_point(ggplot2::aes(nearest_gene_name, Clone_contribution*100, color=Sample), size=5, alpha=0.6, shape=18)+
      ggplot2::theme_bw()+
      ggplot2::ylim(0, base::max(immune_sheet$Clone_contribution*100*1.1, 5, na.rm = TRUE))+
